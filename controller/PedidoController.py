...
def criar_pedido():
    try:
        ...
        numero_telefone =usuario_dao.numero_telefone(telefone=usuario_data.telefone)
        ...
        mensagem = (
            f"━━━━━━━━━━━━━━━━━━━━━━🛎️ *Novo Pedido Realizado!* ━━━━━━━━━━━━━━━━━━━━━━\n\n"
            f"🆔 *Pedido:* #{pedido_id}\n"
            f"👤 *Cliente:* {nome_completo}\n"
            f"📞 *Telefone:* {numero_telefone}\n"
            f"📍 *Endereço:* {endereco_completo}\n"
            f"💳 *Pagamento:* {forma_pagamento}\n"
            f"\n 🕐 *Tempo de entrega:* 40 a 70 minutos"
            f"\n 📦 *Status:* Preparando\n"
            f"\n 🍔 *Itens Comprados* 🍔:\n\n" + ''.join([f"• {item}" for item in itens_comprados]) +
            f"\n*❗ Observações ❗:*\n\n" + '\n'.join([f"❗{obs}" for obs in observacao_item]) +
            f"\n\n"
            f"💰 *Total:* R$ {valor_total:.2f}"
        )


        # Codifica corretamente para UTF-8 e mantém os emojis
        mensagem_codificada = quote(mensagem, safe='')
        

        numero_telefone = "5521979844840"
        link_whatsapp = f"https://api.whatsapp.com/send?phone={numero_telefone}&text={mensagem_codificada}"


        session['link_whatsapp'] = link_whatsapp
